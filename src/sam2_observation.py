...
def match_frame_inputs(rgb_dir: Path, sam2_dir: Path) -> list[Sam2FrameInput]:
    ...
    metadata_files = {
        frame_id: path
        for frame_id, path in _files_by_frame(sam2_dir, ".json").items()
    ...
    return [
        Sam2FrameInput(
            ...
            metadata_path=str(metadata_files[frame_id]),
            label_image_path=str(label_files[frame_id]),
        )
        for frame_id in sorted(frame_ids)
    ]


def _read_label_counts(label_image_path: Path) -> tuple[tuple[int, int], dict[int, int]]:
    with Image.open(label_image_path) as image:
        if len(image.getbands()) != 1:
            raise ValueError(f"label image must be single-channel: {label_image_path}")
        ...
        return image.size, counts


def _read_rgb_size(rgb_path: Path) -> tuple[tuple[int, int] | None, str | None]:
    try:
        with Image.open(rgb_path) as image:
            return image.size, None
    except UnidentifiedImageError as error:
        try:
            reference = rgb_path.read_text(encoding="utf-8").strip()
        except (OSError, UnicodeDecodeError):
            raise ValueError(f"cannot decode RGB image: {rgb_path}") from error
...
def _load_metadata(metadata_path: Path) -> list[dict]:
    ...
    if not isinstance(payload, list):
        raise ValueError(f"SAM2 metadata must be a JSON array: {metadata_path}")
    if not all(isinstance(record, dict) for record in payload):
        raise ValueError(f"SAM2 metadata records must be JSON objects: {metadata_path}")
    return payload
...
def parse_frame_observation(
    frame_input: Sam2FrameInput,
    background_labels: Iterable[str] = DEFAULT_BACKGROUND_LABELS,
) -> Sam2FrameObservation:
    rgb_path = Path(frame_input.rgb_path)
    metadata_path = Path(frame_input.metadata_path)
    label_image_path = Path(frame_input.label_image_path)
    ...
    if rgb_size is not None and rgb_size != label_size:
        raise ValueError(
            f"RGB and label dimensions do not match for {frame_input.frame_id}: "
            f"rgb={rgb_size}, labels={label_size}"
        )
    ...
    for record in records:
        ...
        bbox = _validate_bbox(record.get("bbox_xywh", ()), width, height)
        ...
        objects.append(
            Sam2InstanceObservation(
                ...
                predicted_iou=(
                    float(record["predicted_iou"])
                    if record.get("predicted_iou") is not None
                    else None
                ),
    ...
    return Sam2FrameObservation(
        ...
        rgb_image_verified=rgb_size is not None,
        rgb_source_reference=rgb_source_reference,
        metadata_path=str(metadata_path),
        label_image_path=str(label_image_path),
        image_width=width,
        image_height=height,
        total_records=len(records),
        zero_area_records=zero_area_records,
        background_filtered_records=background_filtered_records,
        mask_instance_ids=mask_instance_ids,
        unmatched_metadata_ids=sorted(visible_metadata_ids - mask_id_set),
        unmatched_mask_ids=sorted(mask_id_set - visible_metadata_ids),
        objects=objects,
    )
